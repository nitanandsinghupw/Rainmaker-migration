...
namespace Rainmaker.DAL
{
    public static class CampaignAccess
    {
                ...
                using (SqlTransaction transaction = connect.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (campaignID != objCampaign.CampaignID)
                        {
                            ...
                            CreateTables(strMasterDBConn, campaignDBScriptPath, campaignDBName, false, strOtherCommands, false);
                        }
                        transaction.Commit();
                    }
                    catch (Exception ex)
        ...
        public static Campaign CloneCampaign(string strCampaignMasterDBConn, string strMasterDBConn,
            Campaign objCampaign, string campaignDBScriptPath, CloneInfo cloneInfo)
        {
            bool isCampaignCreated = false;
            string campaignDBName = string.Empty;
            using (SqlConnection connect = new SqlConnection(strCampaignMasterDBConn))
                ...
                using (SqlTransaction transaction = connect.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (campaignID != objCampaign.CampaignID)
                        {
                            ...
                            CreateTables(strMasterDBConn, campaignDBScriptPath, campaignDBName, cloneInfo.IncludeResultCodes, strOtherCommands, true);
        ...
        public static ResultCode GetResultCodeByResultCodeID(Campaign campaign, long resultCodeID)
        {
            ...
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    resultCode.Printable = (bool)r["Printable"];
        ...
        public static void CampaignQueryStatusUpdate(Campaign campaign,
           long campaignQueryID, bool isActive, bool isStandby, bool showMessage)
        {
        ...
        private static void CreateTables(string dbConnString, string nodeTableStructFilePath, string campaignDBName, bool isCloneAndExcludeResultCodes, string otherSqlCommands, bool isClone)
        {
            ...
            try
            {
                fileSQL = ReadDataFromFile(nodeTableStructFilePath);
            }
            catch (Exception exp)
            {
                throw exp;
            ...
            //Split file in half: SQL for Node Table creation, SQL for Node Stored Procedures
            string[] strSQL;
            string[] stringSeparator = new string[] { "*^#^a" };
            strSQL = fileSQL.Split(stringSeparator, StringSplitOptions.None);
            
            //if (strSQL.Length != 2)//if split was unsuccessful
            //{
            //    Exception ex = new Exception("Unable to Split Node_#.sql File");
            //    throw ex;
            //}
            ...
            using (SqlConnection conn = new SqlConnection(dbConnString))
            {
                ...
                try
                {
                    strSQL[0] = strSQL[0].Replace("{0}", campaignDBName);
                    string[] cmds = strSQL[0].Replace("GO\r\n", "~").Split('~');
                    totalCmds = cmds.Length;
                    foreach (string cmd in cmds)
                    {
                        if (!isCloneAndExcludeResultCodes || cmd.IndexOf("insert into resultcode values (") < 0)
                        {
                            if (!isClone || cmd.IndexOf("EXEC dbo.CreateDefaultQuerie") < 0)
                            {
                                // Inserted 09.12.10 for clone query overlap.
                                cmdIndex++;
                                cmdCurrent = cmd;
                                if (cmd.Length > 1)
                                {
                                    int result = SqlHelper.ExecuteNonQuery(dbConnString, CommandType.Text, cmd);
                                }
                            }
                        }
                    }
                ...
                {
                    DebugLogger.Write(string.Format("Exception running clone script at command {0} of {2} - '{1}'.", cmdIndex, cmdCurrent, totalCmds));
                    throw exp;
                }
