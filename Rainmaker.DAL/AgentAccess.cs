...
namespace Rainmaker.DAL
{
    public static class AgentAccess
    {
        ...
        /// <param name="campaignMasterDBConn"></param>
        /// <returns></returns>
        public static Agent InsGet_AgentActivity(Agent agent, string campaignMasterDBConn, string ipAddress, string hostName)
        {
        ...
        public static Agent AgentActivity_AgentID(string campaignMasterDBConn,
            long AgentID)
        {
            //DebugLogger.Write("got here AgentID is :" + AgentID);
            Agent agent = new Agent();
            ...
            for (int i = 1; i < m_MaxTransactionRetries; i++)
            {
                try
                {
                    ...
                    //DebugLogger.Write("got here2");
                    ds = SqlHelper.ExecuteDataset(campaignMasterDBConn, CommandType.StoredProcedure,
                             "AgentActivity_AgentID", sparams);
                    if (ds.Tables[0].Rows.Count == 1)
                    {
                        DataRow r;
                        r = ds.Tables[0].Rows[0];
                        ...
                        agent.LoginTime = Convert.ToDateTime(r["LoginTime"] == Convert.DBNull ? DateTime.MinValue : Convert.ToDateTime(r["LoginTime"]));

                        agent.LogoutTime = Convert.ToDateTime(r["LogoutTime"] == Convert.DBNull ? DateTime.MinValue : Convert.ToDateTime(r["LogoutTime"]));
                     
                        agent.StationHost = Convert.ToString(r["StationHostName"] == Convert.DBNull ? "" : Convert.ToString(r["StationHostName"]));
                ...
                catch (Exception ex)
                {
                    if (ex.Message.IndexOf("Rerun the transaction") > 0)
                    {
                        if (i < m_MaxTransactionRetries)
                        {
                            DebugLogger.Write("Transaction in AgentActivity_AgentID failed, re-running SQL transaction.");
                            continue;
                        }
                        else
                        {
        ...
        /// <summary>
        /// Returns record that shows if they have any
        /// active logins
        /// </summary>
        /// <param name="campaignMasterDBConn"></param>
        /// <param name="AgentID"></param>
        /// <returns></returns>
        public static Agent GetAgentActivityByAgentID(string campaignMasterDBConn,long AgentID)
        {
            SqlParameter[] sparam_s = new SqlParameter[]{
                new SqlParameter("@AgentID",AgentID)
            };
            DebugLogger.Write("agent id is :" + AgentID);
            DataSet dsAgentList;
            Agent agent = new Agent();
            for (int i = 1; i <= m_MaxTransactionRetries; i++)
            {
                ...
                {
                    dsAgentList = SqlHelper.ExecuteDataset(campaignMasterDBConn,
                        CommandType.StoredProcedure, "AgentActivity_AgentID", sparam_s);
                    ...
                    if (dsAgentList.Tables[0].Rows.Count == 1)
                    {
                        ...
                        agent = AgentActivity_AgentID(campaignMasterDBConn,agent.AgentID);
                    }
