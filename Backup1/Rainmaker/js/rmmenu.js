...
function Menu(label, mw, mh, fnt, fs, fclr, fhclr, bg, bgh, mborder, miborder, borderbg, ItemBorderColor, LiteBgcolor) {
	...
	this.bgColor = ItemBorderColor || "#555555";
	this.menuBorder = mborder;
	this.menuItemBorder = miborder;
	this.menuItemBgColor = bg||"#cccccc";
	this.menuLiteBgColor = LiteBgcolor ||"#CCCCCC";
	this.menuBorderBgColor = borderbg ||"#777777";
...
function writeMenus(container) {
	...
	for (var i=0; i<container.menus.length; i++, countMenus++) {
		...
		for (var i=0; i<menu.items.length; i++) {
			...
			var dTag	= '<DIV ID="menuItem'+ countItems +'" STYLE="position:absolute;left:0px;top:'+ (i * menu.menuItemHeight) +'px;'+ itemProps +'">';
			var dClose = '</DIV>'
			if (menu.bgImageUp) {
				menu.menuBorder = 0;
				menu.menuItemBorder = 0;
				dTag	= '<DIV ID="menuItem'+ countItems +'" STYLE="background:url('+menu.bgImageUp+');position:absolute;left:0px;top:'+ (i * menu.menuItemHeight) +'px;'+ itemProps +'">';
				if (document.layers) {
					dTag = '<LAYER BACKGROUND="'+menu.bgImageUp+'" ID="menuItem'+ countItems +'" TOP="'+ (i * menu.menuItemHeight) +'px" style="' + itemProps +'">';
					dClose = '</LAYER>';
				}
			}
			...
			var dText	= '<DIV ID="menuItemText'+ countItems +'" STYLE="' + textProps + 'color:'+ menu.fontColor +';">'+ item +'&nbsp</DIV>\n<DIV ID="menuItemHilite'+ countItems +'" STYLE="' + textProps + 'top:1px;color:'+ menu.fontColorHilite +';visibility:hidden;">'+ item +'&nbsp</DIV>';
			if (item == "separator") {
		...
		for (var x=0; x<container.menus.length; x++) {
			...
			for (var i=0; i<container.menus[x].items.length; i++) {
				...
				if (sep) {
					...
					l.isSeparator = true
					top -= (menu.menuItemHeight - l.style.pixelHeight) + 'px';
				} else {
					l.style.cursor = "hand"
				}
				menuCount++;
...
function FW_showMenu(menu, x, y, child) {
	...
	if (menu) {
		...
		hideSelectElements(x,y,menu.menuWidth, menu.menuHeight);
	}
}
