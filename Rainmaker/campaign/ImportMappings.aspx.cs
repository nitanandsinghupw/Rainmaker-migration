...
{
    public partial class ImportMappings : PageBase
    {
        ...
        private Campaign ReadCampaignInfo()
        {
            if (Session["Campaign"] != null)
            {
                ...
                allowSevenDigitNums = objCampaign.Allow7DigitNums;
                allowTenDigitNums = objCampaign.Allow10DigitNums;
        ...
        private void BindCampaignFieldColumn(GridViewRowEventArgs e)
        {
            ...
            BindColumnsDropdown(ddl, new ListItem("-Select-", "-Select-"));
        ...
        protected void lbtnUpload_Click(object sender, EventArgs e)
        {
            if (ValidateDataMapping())
            {
                ...
                if (list.Count > 0)
                {
                    ...
                    Response.Redirect("ImportStats.aspx");
                }
                else
                {
        ...
        {
            Campaign campaign = ReadCampaignInfo();
            using (SqlConnection connection = new SqlConnection(campaign.CampaignDBConnString))
            {
                connection.Open();
        ...
        private void ImportData(SqlCommand command, List<ImportFieldRow> list)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                {
                    command.CommandText = "INSERT INTO [dbo].[Campaign](" + sqlCols + ") values(" + sqlValues + ")";
                ...
                else
                {
                    if (ruleAllowsUpdates())
                    {
                        ...
                        command.CommandText = string.Format("Update [dbo].[Campaign] set {0} where PhoneNum = '{1}'", strUpdate, strPhoneNumber);
                        SysLog.Write("RAINYDAY", command.CommandText);
                        command.ExecuteNonQuery();
        ...
        {
            bool found = false;
            command.CommandText = String.Format("SELECT count(*) FROM [dbo].[Campaign] WHERE PhoneNum = '{0}'", strPhoneNumber);
        ...
        private void CreateCampaignFieldList(SqlCommand command)
        {
            ...
            strSQL += " WHERE table_name='Campaign'";
