CKEDITOR.dialog.add('rmSelectDialog', function(editor) {
...

			        {
			            ...
			            commit: function(element) {
			                ...
			                if (value) {
			                    ...
			                    element.data('name', value.toLowerCase());
									...
									},
									{
									    ...
									    onChange: function() {
									        var dialog = this.getDialog(),
												values = dialog.getContentElement('rmFieldDefinition', 'cmbValue'),
												optName = dialog.getContentElement('rmFieldDefinition', 'txtOptName'),
												optValue = dialog.getContentElement('rmFieldDefinition', 'txtOptValue'),
									    ...
									    commit: function(element) {
									        ...
									        var dialog = this.getDialog(),
												...
												optionsValues = getOptions(dialog.getContentElement('rmFieldDefinition', 'cmbValue')); //,
									        selectValue = dialog.getContentElement('rmFieldDefinition', 'txtValue').getValue();

									        removeAllOptions(element);

									        var name = element.data('rmFieldNameSaved');
									        if (name == 'cboResultCode') {
									            addOption(element, '#RESULT CODE#', -1, dialog.getParentEditor().document);
									        }
									        else {
									            //addOption(element, '#' + name + '#', -1, dialog.getParentEditor().document);
							...
							},
							{
								...
								[
									...
									},
									{
									    ...
									    onChange: function() {
									        var dialog = this.getDialog(),
												names = dialog.getContentElement('rmFieldDefinition', 'cmbName'),
												optName = dialog.getContentElement('rmFieldDefinition', 'txtOptName'),
												optValue = dialog.getContentElement('rmFieldDefinition', 'txtOptValue'),
												iIndex = getSelectedIndex(this);
									    ...
									    setup: function(name, element) {
									        if (name == 'clear')
									            ...
									            if (element.getAttribute('selected') == 'selected')
									                this.getDialog().getContentElement('rmFieldDefinition', 'txtValue').setValue(oValue);
						...
						[
							{
							    ...
							    onClick: function() {
							        ...
							        var dialog = this.getDialog(),
										values = dialog.getContentElement('rmFieldDefinition', 'cmbValue'); //,
							        txtValue = dialog.getContentElement('rmFieldDefinition', 'txtValue');
